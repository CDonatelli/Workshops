...
print(paste("Confidence Intervals:", C1))
...
# One-way ANOVAs
```{r}
show(carStats) #Recall that we alredy calculated summary statistics for mpg. The means differ, but are they significantly different?
...
Anova(model2,type="III") # Get an ANOVA table
anova(model2) # Get an ANOVA table
```
...
q=p+stat_summary(geom = "errorbar", 
  ...
  theme(panel.grid.major = element_blank(),                           #Take away the grid in the plot
        ...
        text=element_text(size=20, family="serif"),                   #Make the rest of the font bigger and Times New Roman
